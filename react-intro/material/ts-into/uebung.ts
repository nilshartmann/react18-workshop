...
declare function createGreeting(): never;
...
// - todo: Passe also die Signatur so an, so dass es keine Fehler mehr gibt
...
const [greetings] = reactUseState([]);
greetings.forEach(g => g.toUpperCase());

const [address] = reactUseState({ city: "Hamburg" });
